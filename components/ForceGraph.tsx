...
// Color mapping for the 9 axes
const AXIS_COLORS: Record<string, string> = {
  love: '#ec4899',       // Pink (Romantic)
  friendship: '#22c55e', // Green (Friends)
  kinship: '#14b8a6',    // Teal (Family)
  trust: '#3b82f6',      // Blue
  fear: '#9333ea',       // Purple
  respect: '#f59e0b',    // Amber
  desire: '#be123c',     // Rose/Red (Lust)
  discipline: '#94a3b8', // Slate (Duty)
  power: '#7f1d1d',      // Dark Red (Dominance)
  default: '#475569'
};
...
const ForceGraph: React.FC<ForceGraphProps> = ({ data, onNodeClick, onLinkClick, width, height }) => {
  ...
  useEffect(() => {
    ...
    // Create arrow markers for each emotion color
    Object.entries(AXIS_COLORS).forEach(([key, color]) => {
        defs.append("marker")
        .attr("id", `arrow-${key}`)
        .attr("viewBox", "0 -5 10 10")
        .attr("refX", 25) // Moved closer to circle
        .attr("refY", 0)
        .attr("markerWidth", 4) // Smaller arrow
        .attr("markerHeight", 4) // Smaller arrow
        .attr("orient", "auto")
        .append("path")
    ...
    const link = svg.append("g")
      ...
      .on("click", (event, d) => {
        ...
        const originalLink = data.links.find(l => l.source === (d.source as any).id && l.target === (d.target as any).id);
        if(originalLink) onLinkClick(originalLink);
      });

    // Interaction Hitbox
    const linkHitbox = svg.append("g")
      .selectAll("path")
      .data(links)
      .join("path")
    ...
    node.append("circle")
      .attr("r", 20)
      .attr("fill", "#0f172a")
      .attr("stroke", d => d3.schemeTableau10[d.group % 10])
      .attr("stroke-width", 3);
    ...
    simulation.on("tick", () => {
      const pathFn = (d: any) => {
        ...
        const isBidirectional = links.some(l => l.source === d.target && l.target === d.source);
        
        if (isBidirectional || d.source === d.target) {
  ...
  return (
    <svg ref={svgRef} width={width} height={height} className="bg-slate-950/50 rounded-lg w-full h-full" />
  );
};
