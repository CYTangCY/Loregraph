...
import { ACTION_LIBRARY, STRATEGY_MAPPING } from "../constants";
...
const getAiClient = () => {
  // Handle config check safely
  const w = window as any;
  const apiKey = (process.env.API_KEY) || (w.LOREGRAPH_CONFIG && w.LOREGRAPH_CONFIG.API_KEY);
  
  if (!apiKey || apiKey === 'YOUR_API_KEY_HERE') {
      alert("Please set your API Key in config.js.");
